...
(function () {
  ...
  const RAW_CSS = String.raw`
     ...
     *     └── .link-bottom-line
     */
    .lark-doc-home .topic-list-item .main-link > .lark-main-link-content {
      display: flex !important;
      flex-direction: column !important;
      justify-content: center !important;
      align-items: stretch !important;
    ...
    /* 清除第二行中 Discourse 分类 badge / tag 的位置与高度干扰 */
    .lark-doc-home
      .topic-list-item
      .main-link
      .lark-main-link-content
    ...
    /* 文档图标附近不保留原站状态小图标 */
    .lark-doc-home .topic-list-item .main-link :is(
      .topic-statuses,
      .topic-status,
      .avatar-flair
    ) {
    ...
    .lark-doc-home
      ...
      .main-link
      .lark-main-link-content
      :is(.title, .title a, .link-top-line a.title) {
    ...
    .lark-doc-home
      ...
      .main-link
      .lark-main-link-content
      :is(.title:hover, .title a:hover, .link-top-line a.title:hover) {
      color: var(--lark-blue-strong) !important;
    }
  ...
  function makeSidebarSearch() {
    ...
    const sections =
      container.querySelector(".sidebar-sections") ||
  ...
  function makeHomeHeading() {
    ...
    if (heading.parentElement !== headerContents) {
      headerContents.appendChild(heading);
    }
  }

  function makeCreateTopicButton() {
    const controlsRoot = document.querySelector(
      ".navigation-container, .list-controls"
    );
    ...
    let slot = navigationContainer.querySelector(
      ":scope > .lark-create-topic-slot"
    );
  ...
  function makeTopicContext() {
    ...
    const topicTitle = document.querySelector(
      "#topic-title h1, #topic-title .fancy-title"
    );
    ...
    let context =
      headerContents.querySelector(".lark-topic-context");
    ...
    const categoryText =
      category?.textContent?.trim().replace(/\s+/g, " ") ||
      "知识库";
  ...
  function getTopicKey() {
    return (
      document.querySelector("#topic-title h1[data-topic-id]")
        ?.dataset.topicId ||
      document.querySelector("#topic[data-topic-id]")
        ?.dataset.topicId ||
  ...
  function getEditLabel(editTitle) {
    const match =
      editTitle?.match(/(\d{4})\s*年\s*(\d+)月\s*(\d+)日/);
  ...
  function markTopicFullNames(enabled = true) {
    for (const names of document.querySelectorAll(".topic-post .names")) {
      ...
      const fullName =
        fullNameNode?.textContent?.trim();

      names.classList.toggle(
        "lark-has-full-name",
  ...
  function getPostInlineMetadata(post) {
    ...
    const parentPost =
      postInfos?.querySelector(":scope > .reply-to-tab");
  ...
  function renderPostInlineMetadata(host, anchor, items) {
    let metadata =
      host?.querySelector(":scope > .lark-post-inline-meta");
    ...
    const signature = items
      .map(({ kind, label, source }) =>
        [
          kind,
          label,
          source.getAttribute("aria-expanded") || ""
        ].join(":")
      )
      .join("|");
    ...
    const sameSources =
      ...
      items.every(
        ({ source }, index) =>
          metadata._larkMetaSources[index] === source
      );
    ...
    for (const { kind, label, source } of items) {
      ...
      item.className =
        `lark-post-inline-meta-item lark-post-inline-meta-${kind}`;
  ...
  function syncPostInlineMetadata(includePostRows = true) {
    for (const post of document.querySelectorAll(
      ...
      if (!includePostRows) {
        names
          ?.querySelector(":scope > .lark-post-inline-meta")
          ?.remove();
      ...
      const displayName =
        names?.classList.contains("lark-has-full-name")
          ? names.querySelector(":scope > .full-name")
    ...
    const author =
      document.querySelector(".lark-doc-author");

    const authorName =
      author?.querySelector(
        ":scope > .lark-doc-author-name"
      );
    ...
    if (firstPost) {
      renderPostInlineMetadata(
        author,
        authorName,
        getPostInlineMetadata(firstPost)
      );
    } else {
      author
        ?.querySelector(":scope > .lark-post-inline-meta")
  ...
  function syncBoostLists(enabled = true) {
    for (const post of document.querySelectorAll(
      ...
      const contents =
        getPostBody(post)?.querySelector(
          ":scope > .post__contents"
        );
      ...
      const cooked =
        contents?.querySelector(":scope > .cooked");
        ...
        .map((bubble) => {
          const button =
            bubble.querySelector(
              ":scope > .discourse-boosts__cooked"
            );
      ...
      const sameSources =
        boostList?._larkBoostSources?.length ===
          sources.length &&
        sources.every(
          ({ button }, index) =>
            boostList._larkBoostSources[index] === button
        );
      ...
      boostList.className =
        "discourse-boosts__post-menu lark-inline-boosts";

      boostList.setAttribute("aria-label", "Boost");
      boostList.dataset.signature = signature;

      boostList._larkBoostSources =
  ...
  function makeTopicAuthor() {
    ...
    const heading =
      titleWrapper?.querySelector(":scope > h1");
    ...
    if (
      ...
      if (
        existing &&
        existing.dataset.topicKey !== topicKey
      ) {
        existing.remove();
      }
    ...
    const userCard =
      sourceUser.getAttribute("data-user-card");
    ...
    if (timeLabel) {
      ...
      time.className =
        "lark-post-inline-meta-item lark-doc-author-time";
  ...
  function makePostStyleToggle(
    postRowsThemed
  ) {
    let button =
      document.querySelector(
        ".lark-post-style-toggle"
      );
    ...
    if (!button) {
      ...
      button.className =
        "lark-floating-toggle lark-post-style-toggle";
    ...
    button.innerHTML = `
      <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
        <path
          d="M5 7h13m0 0-3-3m3 3-3 3M19 17H6m0 0 3 3m-3-3 3-3"
          stroke="currentColor"
          stroke-width="1.8"
  ...
  function makeBackButton() {
    ...
    button.className =
      "lark-floating-toggle lark-back-toggle";
  ...
  function setTopicToolsOpen(
    button,
    isOpen
  ) {
    if (
      isOpen &&
      topicToolsCloseTimer
    ) {
      clearTimeout(
        topicToolsCloseTimer
      );
  ...
  function makeTopicToolsToggle() {
      ...
      document.addEventListener(
        ...
        (event) => {
          ...
          if (
            ...
            target.closest(
              ".topic-navigation, .lark-topic-tools-toggle"
            )
          ) {
            return;
          }
    ...
    navigation.id ||=
      "lark-topic-tools-panel";

    if (
      !navigation.dataset.larkHoverBound
    ) {
      navigation.dataset.larkHoverBound =
        "true";
    ...
    if (!button) {
      ...
      button.className =
        "lark-floating-toggle lark-topic-tools-toggle";
  ...
  function normalizeTopicListRows() {
    for (const mainLink of document.querySelectorAll(
      ".topic-list-item .main-link"
    )) {
      /*
       * 兼容两种状态：
       * 1. Linux DO / Discourse 原始 DOM
       * 2. 已经被本脚本处理过的 DOM
       */
      const topLine =
        mainLink.querySelector(
          ":scope > .link-top-line"
        ) ||
        mainLink.querySelector(
          ":scope > .lark-main-link-content > .link-top-line"
        );

      const bottomLine =
        mainLink.querySelector(
          ":scope > .link-bottom-line"
        ) ||
        mainLink.querySelector(
          ":scope > .lark-main-link-content > .link-bottom-line"
        );
      ...
      /*
       * 强制顺序：标题 -> 分类 / 标签。
       * 不 clone 原节点，原来的链接、事件与 Discourse 数据绑定继续保留。
       */
      if (
        topLine.parentElement !== content ||
        content.firstElementChild !== topLine
      ) {
        content.prepend(topLine);
      }

      if (
        bottomLine &&
        (
          bottomLine.parentElement !== content ||
          topLine.nextElementSibling !== bottomLine
        )
      ) {
    ...
    for (
      ...
      for (
        ...
        of document.querySelectorAll(
          `.topic-list-header ${selector}`
        )
      ) {
        let span =
          cell.querySelector(
            ":scope > .lark-column-label"
          );
  ...
  function applyTheme() {
    ...
    const postRowsThemed =
      !isTopic ||
      getPostRowsMode() !== "native";
